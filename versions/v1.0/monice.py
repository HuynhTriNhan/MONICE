...
class GowerDistance(DistanceMetric):
    ...
    def distance(self, X1: np.ndarray, X2: np.ndarray) -> np.ndarray:
        ...
        # Numerical features
        if self.num_feats:
            num_diff = np.abs((X2[:, self.num_feats].astype(np.float64) - 
                              X1[0, self.num_feats].astype(np.float64)) / self.scale)
            
            if self.cost_weights:
                num_weights = np.array([self.cost_weights.get(feat_idx, 1.0) 
                                       for feat_idx in self.num_feats], dtype=np.float64)
                weighted_num_diff = num_diff * num_weights
                total_distance += np.sum(weighted_num_diff, axis=1)
                total_weight += np.sum(num_weights)
            else:
                total_distance += np.sum(num_diff, axis=1)
                total_weight += len(self.num_feats)
        ...
        if self.cat_feats:
            ...
            if self.cost_weights:
                cat_weights = np.array([self.cost_weights.get(feat_idx, 1.0) 
                                       for feat_idx in self.cat_feats], dtype=np.float64)

                weighted_cat_diff = np.dot(cat_diff.astype(np.float64), cat_weights)
                total_distance += weighted_cat_diff
                total_weight += np.sum(cat_weights)
            else:
                total_distance += np.sum(cat_diff, axis=1)
                total_weight += len(self.cat_feats)
...
class NSGAIIOptimizer:
    ...
    def fast_non_dominated_sort(self, objectives_matrix: np.ndarray) -> List[List[int]]:
        ...
        domination_count = [0] * n # solution i is dominated by how many solutions
        dominated_solutions = [[] for _ in range(n)] # the index of solutions that i dominates
        fronts = [[]]
        ...
        for i in range(n):
            for j in range(n):
                if i == j:
                    continue
                if self._dominates(objectives_matrix[i], objectives_matrix[j]):
                    dominated_solutions[i].append(j)
                elif self._dominates(objectives_matrix[j], objectives_matrix[i]):
                    domination_count[i] += 1
            
            # First front contains non-dominated solutions
...
class ConstrainedMultiObjectiveOptimizer:
    ...
    def optimize(self, X_original: np.ndarray, nearest_neighbors: np.ndarray, 
        ...
        for generation in range(self.max_generations):
            ...
            if offspring.shape[0] > MAX_OFFSPRING:
                selected_indices = np.random.choice(offspring.shape[0], MAX_OFFSPRING, replace=False)
    ...
    def _initialize_population(self, nearest_neighbors: np.ndarray, 
        ...
        # Nearest neighbors
        for i in range(n):
            NN_population.append(nearest_neighbors[i:i+1, :])
        NN_population = np.concatenate(NN_population, axis=0)
        
        # Uniform crossover between nearest neighbors (p=0.5) and interpolation-based offspring with numerical steps of 0.5 and 1.0
        for i in range(n-1):
            for j in range(i+1, n):
                ...
                for feat_idx in range(parent_1.shape[1]):
                    if np.random.random() < 0.5:
                        uniform_offspring[0, feat_idx] = parent_2[0, feat_idx]
        ...
        population = self._select_valid_counterfactual(population, X_original, target_class)
                     
        if population.shape[0] > MAX_OFFSPRING:
            selected_indices = np.random.choice(population.shape[0], MAX_OFFSPRING, replace=False)
            population = population[selected_indices]
        
        population = np.concatenate([population, NN_population], axis=0)
        population = self._remove_duplicates(population)
    ...
    def _generate_offspring(self, parent_1: np.ndarray, parent_2: np.ndarray, numerical_steps: np.ndarray) -> np.ndarray:
        ...
        num_diff_features = [f for f in diff_features if f in self.monice.num_feats]
        cat_diff_features = [f for f in diff_features if f in self.monice.cat_feats]
    ...
    def _calculate_objectives(self, X_cfs: np.ndarray, X_original: np.ndarray, 
        ...
        for i, objective in enumerate(self.objectives):
            if objective == 'robustness':
                ...
                probs = self.monice.predict_fn(X_cfs)
    ...
    def _remove_duplicates(self, candidates: np.ndarray) -> np.ndarray:
        ...
        # This code for removing duplicates when the feats are not all numerical
        # seen = set()
        # unique_rows = []
        ...
        #         seen.add(row_tuple)
        #         unique_rows.append(candidates[i])

        # unique_array = np.array(unique_rows, dtype=candidates.dtype)
        # return self.monice.num_as_float(unique_array)
...
class MONICE:
    ...
    def __init__(self, 
        ...
        self.cat_feats = cat_feats
        self.num_feats = num_feats
        self.integer_feats = integer_feats
    ...
    def _calculate_quality_metrics(self, X_cfs: np.ndarray, X_original: np.ndarray, 
        ...
        for i in range(n_cfs):
            cost = 0.0
            diff_features = X_cfs[i] != X_original[0]
            
            for feat_idx in np.where(diff_features)[0]:
                weight = self.cost_weights.get(feat_idx, 1.0)
                    
                if feat_idx in self.cat_feats:
                    cost += weight
                else:
                    cost += weight * np.abs(X_cfs[i, feat_idx] - X_original[0, feat_idx])
                    
            cost_scores.append(float(cost))
        metrics['cost'] = cost_scores
    ...
    def explain(self, 
        ...
        for target_class in target_classes:
            ...
            for i in range(counterfactual.shape[0]):
                cf = counterfactual[i:i+1, :]
                satisfied = self._check_constraints_satisfied(cf, X)[0]
